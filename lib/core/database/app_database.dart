...
import '../../features/training/data/datasources/converters/execution_context_fallback_converter.dart';
...
class AppDatabase extends _$AppDatabase {
    ...
    onUpgrade: (m, from, to) async {
      ...
      if (from < 42) {
        await _addColumnIfNotExists(
          ...
          column: 'workout_name_snapshot',
          definition: 'TEXT',
        );
        await _addColumnIfNotExists(
          table: 'workout_executions',
          column: 'program_name_snapshot',
          definition: 'TEXT',
        );
        await _addColumnIfNotExists(
          table: 'workout_executions',
          column: 'context_fallback',
          definition: 'TEXT',
        );
        await customStatement('''
          UPDATE workout_executions
          SET workout_name_snapshot = (
            SELECT w.name FROM workouts w
            WHERE w.id = workout_executions.workout_id
          )
          WHERE finished_at IS NOT NULL
            AND workout_name_snapshot IS NULL
        ''');
        await customStatement('''
          UPDATE workout_executions
          SET program_name_snapshot = (
            SELECT p.name FROM programs p
            WHERE p.id = workout_executions.program_id
          )
          WHERE finished_at IS NOT NULL
  ...
  Future<void> _backfillExecutionContextFallbackV42() async {
    final executions = await customSelect('''
    ...
    for (final row in executions) {
      ...
      for (final exerciseId in allExerciseIds) {
        final exerciseRow = await customSelect(
