...
Future<void> _seedDevProfileEquipment(AppDatabase db) async {
  ...
  if (existing != null) {
    await (db.update(db.userProfiles)..where((p) => p.id.equals(existing.id)))
        .write(UserProfilesCompanion(ownedEquipmentNames: Value(owned)));
  } else {
    await db
...
Future<void> _seedExecutionHistory(
  ...
  await _insertSetsWithDropSet(
    ...
    planned: 12,
    normalSets: [(weight: 25.0, reps: 12), (weight: 25.0, reps: 11)],
    dropSetPrimary: (weight: 25.0, reps: 10),
    dropSegments: [(weight: 17.5, reps: 8), (weight: 10.0, reps: 10)],
  );
  ...
  await _insertCompletedSets(
    db,
    exec2,
    exerciseIds['bentOverRow']!,
    planned: 8,
    weights: [60, 60, 62.5, 62.5],
    reps: [8, 8, 7, 7],
  );
  await _insertCompletedSets(
    db,
  ...
  await _insertCompletedSets(
    db,
    exec3,
    exerciseIds['backSquat']!,
    planned: 8,
    weights: [100, 100, 105, 105],
    reps: [8, 8, 6, 5],
  );
  await _insertCompletedSets(
    db,
  ...
  await _insertCompletedCardioSets(
    db,
    exec4,
    exerciseIds['jumpRope']!,
    durations: [180, 165, 150],
    distances: [null, null, null],
  );
  await _insertCompletedCardioSets(
    db,
    exec4,
    exerciseIds['jumpingJacks']!,
    durations: [60, 55, 50],
    distances: [null, null, null],
  );
}
