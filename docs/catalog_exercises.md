...
For conflict governance (import, verified vs local, future multi-device rules), see [catalog_governance.md](./catalog_governance.md). This doc covers **shipping exercise rows inside the app** only.
...
2. **User-facing titles are never the raw key** — verified rows are labeled via ARB (`l10n/*.arb`) and [DomainLabelResolver](../lib/core/localization/domain_label_resolver.dart) (`DomainLabelKind.exercise`, `_exerciseNameMap`). Non-verified (custom) exercises display their stored name as-is.
...
| Localized display names | `lib/l10n/app_*.arb` + `_exerciseNameMap` in [domain_label_resolver.dart](../lib/core/localization/domain_label_resolver.dart) |
...
4. Register the key in `_exerciseNameMap` inside `domain_label_resolver.dart`.
...
6. Run `dart run build_runner build` if generated code is affected, then `flutter analyze` / relevant tests (backup import tests cover `resolveImportedExerciseCatalogName`).
...
3. **Resolver** — for **legacy** keys still present in old backups, keep a row in `_exerciseNameMap` mapping the old key to the **same** user-facing string as the keeper (see comments for `ezBarCurl`, `ropeTricepsPushdown`, etc.).
